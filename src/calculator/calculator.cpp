      ...
      argument_formula_("")
{
  ...
  arithmetic_argument_subscriber_ = this->create_subscription<ArithmeticArgument>("arithmetic_argument",QOS_RKL10V,
  [this](const ArithmeticArgument::SharedPtr msg)->void
  {
      argument_a_ = msg->argument_a;
      argument_b_ = msg->argument_b;

      RCLCPP_INFO(this->get_logger(), "Timestamp of the message: sec %ld anosec %ld", msg->stamp.sec, msg->stamp.nanosec);
  ...
  auto get_arithmetic_operator =
   ...
   const std::shared_ptr<ArithmeticOperator::Request> request,
          std::shared_ptr<ArithmeticOperator::Response> response) ->void{
             argument_operator = request->arithmetic_operator;
  ...
  arithmetic_argument_server_ = create_service<ArithmeticOperator>("arithmetic_operator", get_arithmetic_operator);
...
float Calculator::calculate_given_formula(const float &a. const float &b, const int8_t & operators)
{
    ...
    ArithmeticOperator::Reqeust arithmetic_operator;
...
rclcpp_action::GoalResponse Calculator::handle_goal(const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const ArithmeticChecker::Goal>goal)
{
    (void)uuid;
    (void)goal;
    return rclcpp_action::GoalResponce::ACCEPT_AND_EXECUTE;
}
...
void Calculator::execute_checker(const std::shared_ptr<GoalHandleArithmeticChecker> goal_handle)
{
    ...
    while((total_sum < goal_sum) && rclcpp::ok()){
        ...
        goal_handle->publish_back(feedback_msg);
        loop_rate.sleep();
    }
